...
recording serves the whole scan and each run adds the label of its AP.
  ...
  find_scans()    groups the run directories into scans, and raises if a scan
                  lacks the run for one of its APs.
  find_options()  given a scan's projected recording, returns the APs with a
                  decoded beacon, each with its channel and the label of the run
                  that joined it.
"""
...
from scripts.dataset.projection import ProjectedScan


@dataclasses.dataclass(frozen=True)
class Scan:
    """One client position: the run that joined each AP."""
...
@dataclasses.dataclass(frozen=True)
class Option:
    """An AP the client could join, and what joining it gave."""
...
def find_options(scan: Scan, projected: ProjectedScan) -> list[Option] | None:
    ...
    for index, run in scan.runs.items():
        ...
        if mac in channel_of:
            options.append(Option(index, mac, channel_of[mac],
                                  metadata["candidate"]["throughput_mbps"],
                                  bool(metadata["candidate"]["associated"]), metadata))
    if len(options) < 2:
        print(f"WARN: scan {scan.scan_id} discovered only {len(options)} AP(s); skipping",
              file=sys.stderr)
        return None
    return options
