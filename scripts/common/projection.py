...
PROCESS, in memory, in project()
  ...
  2. Schedule a sweep: one radio dwells on channels 36, 40, 44 and 48 in a
     random order, 110 ms per dwell, for as many whole passes as the window
     holds, the last dwell ending at the window end.
  ...
  whole window. The channel order is drawn from SEED and the scan ID, so every
...
# One dwell on a channel, and the deaf time at its start while the radio retunes.
DWELL_MS = 110.0
RETUNE_MS = 0.15
...
def sweep_rng(scan_id: str) -> random.Random:
    """The generator for one scan's channel order, seeded from a digest of SEED and
    the scan ID.
...
def dwell_of(time_s: float, sweep_start_ms: float) -> int:
    """The number of the dwell a data point falls in, 0 for the first dwell of
    the sweep.

    A data point is a decoded frame or a carrier-sense millisecond, placed by
    the moment it started. The number says which dwell heard it, so that later
    steps can group data points by dwell. A moment outside the sweep gives a
    number outside the schedule.
    """
    # floor, not int(): int() truncates toward zero, so a moment just before
    # the sweep would land in dwell 0
    return math.floor((time_s * 1000.0 - sweep_start_ms) / DWELL_MS)


def dwell_bounds(dwell: int, sweep_start_ms: float) -> tuple[float, float, float]:
    """When a dwell starts, when the radio is done retuning and starts listening,
    and when the dwell ends, in seconds.

    The edges are counted in milliseconds and divided by 1000 only here, so
    every caller gets the same float for the same edge, and a whole-millisecond
    edge is the same float as that millisecond read from chanbusy.csv.
    """
    start_ms = sweep_start_ms + dwell * DWELL_MS
    return (start_ms / 1000.0, (start_ms + RETUNE_MS) / 1000.0,
            (start_ms + DWELL_MS) / 1000.0)
...
def in_dwell(channel: int, start: float, end: float, dwell_schedule: list[int],
    ...
    """
    dwell = dwell_of(start, sweep_start_ms)
    if not (0 <= dwell < len(dwell_schedule)) or channel != dwell_schedule[dwell]:
        return False
    _, listen_start, dwell_end = dwell_bounds(dwell, sweep_start_ms)
    return start >= listen_start and end <= dwell_end
...
class ProjectedScan:
    ...
    sweep_start_ms: float             # when the first dwell starts, in milliseconds
...
def project(run_dir: Path, scan_id: str, all_channels: bool) -> ProjectedScan:
    ...
    busy = [e for e in busy if in_dwell(e["channel"], e["start"], e["end"],
                                        dwell_schedule, sweep_start_ms)]
    return ProjectedScan(window, frames, busy, dwell_schedule, sweep_start_ms)
